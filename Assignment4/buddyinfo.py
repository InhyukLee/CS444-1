...
class BuddyInfo(object):
    ...
    def read_buddyinfo(self):
        ...
        for line in map(self.parse_line, buddyinfo):
            ...
            if ( not ( buddyhash.has_key(numa_node) ) ):
                buddyhash[numa_node] = [] 
        ...
        for node in self.buddyinfo:
            ...
            for zoneinfo in self.buddyinfo.get(node):
                ...
                ret_string += '\t{0:{align}{width}} {1:{align}{width}} {2:{align}{width}}\n'.format(
                        ...
                        align="<")
                """
                ret_string += '%c%-*s %-*s %-*s%c' %("\t",width,"Fragment size",width,"Free fragments",width,"Total available KiB","\n")
                ...
                for idx in range(len(zoneinfo.get("sz_fragment"))):
                    ...
                    """
                    ret_string += '%c%-*s %-*s %-*s%c' %("\t",width,zoneinfo.get("sz_fragment")[idx],width,zoneinfo.get("nr_free")[idx],width,((zoneinfo.get("usage")[idx])/1024.0),"\n")
